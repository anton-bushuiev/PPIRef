...
from ppiref.comparison import FoldseekMMComparator
...
def main(split, fold, pairs, out_dir, max_workers, partition_beg, partition_end, seed,
    ...
    # Init comparator
    foldseek_mm = FoldseekMMComparator(max_workers=max_workers, verbose=verbose)
    if verbose:
        print(f'Initialized comparator with {max_workers} workers')

    # Compare
    df = foldseek_mm.compare_all_against_all(**inputs)
    out_file = out_dir / f'foldseek_mm_comp_{name}_{partition_beg}-{partition_end}{out_file_suff}.csv'
    if verbose:
